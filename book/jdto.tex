...
When used the right way, DTO objects are key on building a scalable and loosely coupled service oriented architecture that can be deployed over the cloud.
...
\item Improved the process of reverse binding by adding an optional "restore" functionality to the value mergers.
\item Bean modifiers now can transparently convert between dates and calendars.
\item Added restore functionality to four of the property mergers.
\item Added a new merger to calculate the time between two dates in different time units.
\item Introduced new marker interface to generalize the value mergers.
\item Value merger instances can be obtained directly by the user to take advantage of their functionality.
...
\item Circular reference mapping should now not be an issue on non-immutable DTOs.
\item Now the @DTOCascade annotation can be used within a constructor argument.
\item Fixed small analysis issue on immutable DTOs with the cascade option.
\item Fixed test that would not pass under certain environments.
...
\begin{itemize}
\item Commons Lang (min version 2.6): http://commons.apache.org/lang/
