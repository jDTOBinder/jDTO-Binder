...
\begin{itemize}
\item Default location for configuration file: now, by default, the framework looks for a package resource file located at \DEFAULTCONFIG.
\item Map Binding: Users can populate \texttt{Map} implementations from objects and populate objects from maps.
\end{itemize}

Issues solved:
\begin{itemize}
\item Fixed \texttt{NullPointerException} when no DTO entries are specified on a XML file (which is valid).
\end{itemize}
